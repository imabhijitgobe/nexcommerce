---
name: develop
description: Use when building a NexCommerce feature from a PRD story (US-001 to US-070) and docs/api.md contract: implements UI and backend code, ticks docs/todos.md boxes. Triggers on develop, build, implement feature.
---
...
The governing spec is the PRD story plus the API contract: `docs/prd.md` user story (US-001..US-070, with acceptance criteria) and requirements (FR-001..FR-065), plus `docs/api.md` (endpoint, request/response, error codes) for the matching section. The todos task (`docs/todos.md`, ID like AUTH-001) names both. Conventions live in `docs/git-conventions.md` (commit format `type(scope): description`, scopes: auth, user, vendor, product, search, order, payment, delivery, notification, review, analytics, ai, admin, api, db, infra, ci, deps, docs, config) and `docs/error-codes.md` (branch UI and handling on `code`, never on message). UI screens and routes come from `docs/ui-flows.md`; env keys from `docs/env.md`. There is no `design.md`; `docs/ui-flows.md` is the UI source. No `docs/scope/` exists; scope updates below mean ticking `docs/todos.md` boxes and mirroring `docs/progress.md`.
...
- Todos (`docs/todos.md`): Step 4 ticks only the task boxes this build completed (verified by its Verify line), then mirrors module status in `docs/progress.md`. Never tick a box whose Verify did not pass; never tick other tasks. Never creates todos files (planning is `/scope`'s; analysis/research is `/architect`'s, in the spec's `rationale.md`).
...
**Check, in order:**
1. **Locate this feature's todos tasks (only those).** Find the task IDs (e.g. AUTH-001) by scanning `docs/todos.md` headings, or from the prompt itself. Read just those task lines plus their Verify lines. A task blocked on an undecided load bearing choice with no spec pointer yet → decision owed and missing. Malformed → flag and ask, don't guess.
