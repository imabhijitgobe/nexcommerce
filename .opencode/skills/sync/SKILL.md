---
name: sync
description: Use after a NexCommerce change completes: ticks docs/todos.md from repo evidence, mirrors docs/progress.md, maintains AGENTS.md, flags stale specs. Triggers on sync, reconcile, update todos, post-merge cleanup.
---
...
Scope reconciliation target is `docs/todos.md` (tick boxes verified from repo evidence) plus the `docs/progress.md` mirror, not `docs/scope/`. Tick any todos sub task verifiable from the diff (migration applied, endpoint exists and matches `docs/api.md`, test passes) and advance the progress row; exact rules in `agent-prompt.md` still apply (surgical, additive, durable). Flag stale PRD coverage (a US story with no todos task) to `/scope`, and spec staleness to `/architect` (`docs/specs/` once it exists).
...
| Reconcile the todos, for the **relevant Wave/Phase section(s)** only (not all of `docs/todos.md`), tick **any** completed sub task from repo **evidence** (code, tests, AGENTS.md), advance status | ✅ corrects | /sync |
...
- Find the todos section(s) whose features the diff actually touches (match task IDs or Wave headings in `docs/todos.md`); never read or pass all of `docs/todos.md` when only one Wave is affected.
...
The inputs to apply:
  ...
  6. `SCOPE_PATH_OR_NONE` (relevant todos Wave section(s), not all of `docs/todos.md`; also the source of each linked feature's status for spec Status line reconciliation)
