---
name: test
description: Use after building NexCommerce code to write Jest or Vitest suites tagged covers US-xxx, tracing to PRD acceptance criteria. Triggers on test, write tests, test suite, coverage.
---
...
Stack: Node.js with Express, Jest or Vitest plus Supertest for API tests, Playwright for E2E page flows (`docs/architecture.md`). Default `test-preferences.json` when missing: unit tool Jest (or Vitest if the repo uses it), E2E Playwright only when `docs/ui-flows.md` page/flow files changed. Traceability: tag each test with the PRD story it covers (e.g. `covers: US-031`) and the todos ID (e.g. `AUTH-013`); `NOT_COVERED` entries defer to `/check verify` manual steps. Error assertions use `docs/error-codes.md` codes. Commits for suites use `test(<scope>): …` with scopes from `docs/git-conventions.md` (auth, user, vendor, product, search, order, payment, delivery, notification, review, analytics, ai, admin).
...
- Identify the governing spec: the feature dir `docs/specs/NNNN-<feature>/` (or single `docs/specs/NNNN-<feature>.md`) these files implement, matched by branch/feature name or touched surfaces (a `docs/todos.md` task line, if present, names the US story and spec pointer). Note its path and whether a `verify.md` sits beside it (`docs/specs/NNNN-<feature>/verify.md`). This contract is what tests trace to; it may not be among the 3 recent paths. Set `TRACE_TO_CONTRACT = yes` when a governing spec exists, else `no` (fall back to the PRD story acceptance criteria in `docs/prd.md`).
...
Update the todos: if this feature has `docs/todos.md` tasks and the suite passes, tick the test task boxes, then **offer `done`, don't gate it**: "Tests are in and passing, mark it `done`?" On the engineer's go, tick remaining boxes for the feature and mirror `docs/progress.md`. An `Assumed` spec does not block `done`; flag it ("owes ratification, `/architect` when you can") and let them decide. If tests fail or coverage is partial, leave test boxes unticked and report. **Confirm the update as a closing gate** (don't skip it): report exactly what you ticked in each file, e.g. "Todos: ticked AUTH-013, AUTH-014. Progress: Auth 3/30." No matching todos tasks → say so, don't finish silently. On `done`, advise `/clear` before the next feature: the todos and spec hold everything, a fresh session keeps the next build cheap. **Git:** if the nearest `AGENTS.md` `## Git` says `integration: on` and `commit` is not `manual`, offer to commit the suite with a one line subject (`test(<scope>): …`) plus the `Co-Authored-By` trailer; never push. (Effective tier = the feature's own tier tag if set, else the project `**Workflow:**` default. `/test` is the closer at `Beta`/`GA`; `Prototype` closes at `/develop`, `Alpha` at `/check verify`, so on those tiers this feature is already `done` or does not use `/test`. Honor an override tag; never default to a fixed chain.)
