---
name: debug
description: Use when a NexCommerce bug needs root-causing: reproduce, localize, hypothesize, fix, verify loop with minimal fix plus regression test. Triggers on debug, bug, failing test, root cause. Maps symptoms to docs/error-codes.md first.
---
...
Map every symptom to `docs/error-codes.md` first: the code (e.g. `ORDER_INVENTORY_FAILED`, `PAYMENT_WEBHOOK_SIGNATURE_INVALID`) tells you which module owns the bug and which handler to suspect; branch every assertion on `code`, never on message text. Known hot spots from the spec: saga compensating transactions (`docs/prd.md` FR-027), Stripe webhook arriving before the API response (upsert/lock race, FR-031), inventory lock contention (FR-018), Pinecone/ES fallback paths. The regression test traces to the PRD story (US-xxx) and todos ID; hand it to `/test` with both cited.
