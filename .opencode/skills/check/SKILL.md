---
name: check
description: Use before merging NexCommerce changes: verify drives the running app against PRD acceptance criteria, review runs a fresh-model code review into docs/reviews/. Triggers on check, verify, review, pre-merge check.
---
...
The spec contract is the PRD story: `docs/prd.md` user story (US-001..US-070) acceptance criteria plus `docs/api.md` endpoint contract (request/response shapes, error codes). `verify` proves each acceptance criterion against the running app and each specced surface (route from `docs/ui-flows.md`, table from `docs/architecture.md` Section 7, migration applied) exists. Findings reports go to `docs/reviews/` (create the dir on first review). Scope ticks below mean `docs/todos.md` boxes plus `docs/progress.md` mirror. Error assertions use `docs/error-codes.md` codes (branch on `code`, never on message).
