---
name: document
description: Use to write PR text in type(scope) format, CHANGELOG entries, release notes, or postmortems from the real NexCommerce diff. Triggers on document, PR description, changelog, release notes, postmortem.
---
...
PR titles MUST follow `docs/git-conventions.md`: `type(scope): short description`, imperative, lowercase, 72 chars max, scopes auth, user, vendor, product, search, order, payment, delivery, notification, review, analytics, ai, admin, api, db, infra, ci, deps, docs, config. PR bodies link the PRD story (US-001..US-070), the todos IDs closed, and refs from the commits. `changelog` appends to `CHANGELOG.md` at repo root (create it if missing, Keep a Changelog format). `release-note` writes `docs/releases/<version>.md`. `postmortem` writes `docs/postmortems/<date>-<slug>.md`.
