# Scope Mode: replan (NexCommerce: reconcile docs/todos.md + docs/progress.md)

## Replan (the living rhythm, run after a feature or phase ships)

The default cadence, not rare: run each time a feature or phase lands, keeping the todos matching reality and queueing the next slice. Reconciles in place, never spawns a new file; coarse and surgical (tick boxes, reconcile cells, append tasks, don't rewrite the file).

1. Read `docs/todos.md` again (whole file; use the Appendix A task index to jump) plus `docs/progress.md`, and the code/specs for what just shipped.
2. Reconcile what shipped: tick todos boxes whose Verify passes against the code (verify from code/spec, don't stamp); tick nothing unconfirmed; leave boxes `/develop`/`/sync` already advanced. Mirror module status into `docs/progress.md` (same run, never a separate pass). **A feature whose governing spec is `Assumed`** keeps its `assumed decision (spec NNNN)` note and is surfaced (below) as decision debt; the note does not block `done`, so reconcile its status from what actually shipped.
