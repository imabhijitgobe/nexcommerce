...
> NexCommerce note: this project is a modular monolith in a single repo (ADR-001 in `docs/architecture.md`), not a monorepo. Skip this route unless workspace markers (`pnpm-workspace.yaml`, `turbo.json`, `apps/*/package.json`) actually appear. The generic rules below are kept for that case.
