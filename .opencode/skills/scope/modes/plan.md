...
### Step 6: Write the todos (single file, in place)

List `docs/todos.md` again immediately before writing (a teammate may have changed it), then write per the todos format (task IDs `<PREFIX>-<NNN>`, one action plus a `Verify:` line each, grouped under Phase/Wave sections):
