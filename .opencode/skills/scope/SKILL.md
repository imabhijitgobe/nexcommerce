---
name: scope
description: Use when planning NexCommerce work in docs/todos.md and docs/progress.md: planning the next wave, enrolling a feature as new todos tasks, or reconciling shipped work. Triggers on scope, plan, todos, roadmap, what to build next.
---

## NexCommerce adaptation (read first)

This project does not use `docs/scope/`. The living plan is `docs/todos.md` (spec driven task list, 504 tasks with IDs like AUTH-001, each with a Verify line) and the status table is `docs/progress.md` (module status M0-M9, milestones M0-M9). Wherever this skill says `docs/scope/`, read `docs/todos.md` (task level) plus `docs/progress.md` (module level). Wherever it says "feature row", read "todos task(s)"; wherever it says "At a glance table", read the progress dashboard plus the Appendix A task index. Specs live in `docs/specs/` (created by /architect; does not exist yet, first spec creates it). PRD stories US-001..US-070 and requirements FR-001..FR-065 in `docs/prd.md` are the acceptance source every Done when line traces to.
...
## Artifact ownership (NexCommerce)

`docs/todos.md` is the feature scope, owned by this skill; `/architect` owns `docs/specs/`. Other skills find a feature by scanning `docs/todos.md` for its task IDs. Living document: `plan`, `replan`, `add` all edit in place (reconcile and append, never a new dated file). Writes nothing else: no specs, code, or `AGENTS.md`. `docs/progress.md` holds module status only; task level truth lives in `docs/todos.md` checkboxes, never duplicated into progress (progress rows point at todos ID ranges).

File shape (NexCommerce: there is no `docs/scope/`; these shapes describe `docs/todos.md` organization if it ever splits):
- Small product: one file, `docs/todos.md` (task checkboxes grouped by Phase/Wave plus Appendix A index).
- Large product: epic split only if todos outgrows a comfortable scan: keep the index plus one section per epic named by area (auth, orders, …).
...
### Step 0: Infer intent & idea check (NexCommerce)

No subcommand. First check whether todos exist (`docs/todos.md` task checkboxes plus `docs/progress.md` milestones M0-M9), then infer:
- Todos exist + no argument (or running again, described as "reconcile / what's next") → replan behavior (`modes/replan.md`).
- Todos exist + argument names a single feature or module → add behavior (`modes/add.md`).
- No todos yet + a product sized idea, or scoping the next slice (including brownfield) → plan behavior, below.

There is no `docs/scope/` in this project; ignore references to it in route files and read `docs/todos.md` instead.
