...
> NexCommerce note: the living plan already exists as `docs/todos.md` (504 spec tasks with IDs, Verify lines, Wave sections) with status mirrored in `docs/progress.md`. Use this template only when spinning up a brand new epic outside todos; normally you reconcile todos in place instead of writing a scope file. `Done when:` lines trace to `docs/prd.md` stories (US-001..US-070).
...
When a plan outgrows a comfortable scan (roughly a dozen plus features across clearly distinct areas), split by epic into sections: keep the At a glance table across all epics + a one line status rollup per epic, each linking its section. Promote **on demand**; don't split a small product early. File names are always **semantic**, never numbered. (NexCommerce: `docs/todos.md` stays one file; use Wave sections as epics.)
...
Scope written to `docs/todos.md` (tasks appended under the right Wave/Phase; Appendix A index updated).
```
